...
async function getContent() {
  try {
    ...
    // Database not configured/reachable (e.g. missing MONGO_URL, or Mongo not
    // running locally) — serve the built-in default content instead of
    // breaking the page. Admin changes won't persist until the DB is fixed.
    console.error('DB unavailable, serving default content:', e.message);
...
async function verifyToken(req) {
  ...
  try {
    ...
    console.error('DB unavailable for token verification:', e.message);
    // If DB is unavailable, we can't verify tokens - return false
    return false;
  }
}
  ...
  try {
    if (route === '/admin/login') {
      ...
      try {
        ...
        console.error('DB unavailable for session storage, issuing token without persistence:', e.message);
        // Continue anyway - token will work during this session but won't persist across server restarts
      }
      return json({ token });
    }
